...
const get_all_executors_1 = require("./get-all-executors");
const path_1 = require("path");
function setupSchema(workspacePath, schemaVersion, clearPackageJsonCache = false) {
    const collections = get_all_executors_1.getAllExecutors(workspacePath, clearPackageJsonCache);
    const contents = getWorkspaceJsonSchema(workspacePath, schemaVersion, collections);
    return contents;
}
exports.setupSchema = setupSchema;
function getWorkspaceJsonSchema(workspacePath, schemaVersion, collections) {
    if (schemaVersion == 1)
...
function createJsonSchema2(workspaceJsonPath, collections) {
    const basedir = path_1.dirname(workspaceJsonPath);
    const dotIdeaDir = path_1.join(basedir, '.idea');
        ...
        .map((collection) => `
{
  ...
  "properties": {
    ...
    "options": {
      "$ref": "${path_1.relative(dotIdeaDir, collection.path)}"
    },
    "configurations": {
      "type": "object",
...
function createJsonSchema1(workspaceJsonPath, collections) {
    const basedir = path_1.dirname(workspaceJsonPath);
    const dotIdeaDir = path_1.join(basedir, '.idea');
