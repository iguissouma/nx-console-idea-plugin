...
function readDefaultValues(configurations, name) {
    ...
    if (!config)
        return defaults;
    return Object.keys(config).reduce((m, k) => [...m, { name: k, defaultValue: utils_1.getPrimitiveValue(config[k]) }], defaults);
}
async function readBuilderSchema(basedir, builder) {
    const [npmPackage, builderName] = builder.split(':');
    const packageJson = utils_1.readAndCacheJsonFile(path.join(npmPackage, 'package.json'), path.join(basedir, 'node_modules'));
    const b = packageJson.json.builders || packageJson.json.executors;
    const buildersPath = b.startsWith('.') ? b : `./${b}`;
    const buildersJson = utils_1.readAndCacheJsonFile(buildersPath, path.dirname(packageJson.path));
    const builderDef = (buildersJson.json.builders ||
        buildersJson.json.executors)[builderName];
    const builderSchema = utils_1.readAndCacheJsonFile(builderDef.schema, path.dirname(buildersJson.path));
    return await utils_1.normalizeSchema(builderSchema.json);
}
